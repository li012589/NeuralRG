...
if __name__=="__main__":
    ...
    #RNVP block
    depth = 2*int(math.log(Nvars,2))
    print ('depth of the mera network', depth)
    sList = [[MLP(2, args.Hs, activation=ScalableTanh([2])) for _ in range(args.Nlayers)] for l in range(depth)]
    tList = [[MLP(2, args.Ht) for _ in range(args.Nlayers)] for l in range(depth)]
    ...
    model = MERA(1, 2, Nvars, layers, prior, name=key)
