...
class Ising(Target):
    ...
    def energy(self, x): # actually logp
        ...
        + torch.log(torch.cosh(torch.mm(x, self.VT) -0.5*self.d)).sum(dim=1)
    
    def measure(self, x):
        p = F.sigmoid(2.*torch.mm(x, self.VT) -self.d) 
        s = torch.bernoulli(p).data.numpy()
